...
float4 main(PS_INPUT input) : SV_TARGET
{
	...
	float2 uv = input.Tex;
	uv.x *= (1920.0f / 1080.0f);

	//画面を縦にsplitの数だけ分割して(black/split)ずつ増えていく
	float split = 10000.0f;
	float black = 0.01f;

	uv.y += floor(uv.y * split) * (black / split);

	//黒い部分を下にずらしていく
	uv.y += Time.w;

	//透明度
	float alpha = 1.0f - uv.y;

	//透明度の設定
	fade.a = alpha;

//	fade.a = lerp(fade.a, 0.0f, step(0.7f,input.Tex.y));
