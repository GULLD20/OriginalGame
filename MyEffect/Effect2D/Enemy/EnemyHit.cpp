...
const std::vector<D3D11_INPUT_ELEMENT_DESC> EnemyHit::INPUT_LAYOUT =
{
	{ "POSITION",	0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "COLOR",		0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(Vector3), D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT, 0, sizeof(Vector3) + sizeof(Vector4), D3D11_INPUT_PER_VERTEX_DATA, 0 },
};
void EnemyHit::Create(DX::DeviceResources* &deviceResources, int count)
{
	...
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticlePSEnemyHit.cso");
...
void EnemyHit::Initialize(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	int range = 100;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void EnemyHit::InitializeNormal(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
		Vector3 vel=Vector3::Zero;
...
void EnemyHit::InitializeCorn(float life, const DirectX::SimpleMath::Vector3 &pos, const DirectX::SimpleMath::Vector3 &dir)
{
	int range = 10;
	Vector3 vel = Vector3::Zero;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
...
void EnemyHit::Update(const DX::StepTimer &timer)
{
	m_timer = timer;
	for(std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end();itr++)
	{
...
void EnemyHit::SetRenderState(const DirectX::SimpleMath::Vector3 &camera, const DirectX::SimpleMath::Matrix &view, const DirectX::SimpleMath::Matrix &proj)
{
	m_view = view;
	m_proj = proj;
...
void EnemyHit::Draw(const DirectX::SimpleMath::Matrix &world, const DirectX::SimpleMath::Matrix &view, const DirectX::SimpleMath::Matrix &proj)
{
	auto context = m_pDeviceResources->GetD3DDeviceContext();
...
void EnemyHit::SetVector(DirectX::SimpleMath::Vector3 vec)
{
	int range = 1;
	Vector3 vel = Vector3::Zero;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
		Vector3 vel = Vector3(
			(((rand() % (range * 2)) - range)*0.1f / range) + vec.x,
			(((rand() % (range * 2)) - range)*0.1f / range) + vec.y,
			(((rand() % (range * 2)) - range)*0.1f / range) + vec.z);
		vel *= 0.01f;
