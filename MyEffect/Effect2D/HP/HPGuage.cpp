...
const std::vector<D3D11_INPUT_ELEMENT_DESC> HPGuage::INPUT_LAYOUT =
{
	{ "POSITION",	0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "COLOR",		0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(Vector3), D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT, 0, sizeof(Vector3) + sizeof(Vector4), D3D11_INPUT_PER_VERTEX_DATA, 0 },
};
void HPGuage::Create(DX::DeviceResources* &deviceResources, int count)
{
	...
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticlePSHPGuage.cso");
...
void HPGuage::Initialize(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	int range = 100;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void HPGuage::InitializeNormal(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	int range = 1000;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void HPGuage::InitializeCorn(float life, const DirectX::SimpleMath::Vector3 &pos, const DirectX::SimpleMath::Vector3 &dir)
{
	int range = 10;
	Vector3 vel = Vector3::Zero;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
...
void HPGuage::Update(const DX::StepTimer &timer)
{
	m_timer = timer;
	for(std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end();itr++)
	{
	...
	ReduceRedZone(static_cast<float>(timer.GetElapsedSeconds()));
}
...
void HPGuage::SetRenderState(const DirectX::SimpleMath::Vector3 &camera, const DirectX::SimpleMath::Matrix &view, const DirectX::SimpleMath::Matrix &proj)
{
	m_view = view;
	m_proj = proj;
...
void HPGuage::Draw(const DirectX::SimpleMath::Matrix &world, const DirectX::SimpleMath::Matrix &view, const DirectX::SimpleMath::Matrix &proj)
{
	auto context = m_pDeviceResources->GetD3DDeviceContext();
	...
	cbuff.Hp = Vector4(m_maxHp, m_nowHp, m_redZone, 1);
	...
	//定数バッファをシェーダに渡す（PSは要らないのでコメントアウト）
...
void HPGuage::ReduceRedZone(float elestepTime)
{
	m_hpDrawRedCount += elestepTime;

	if (m_hpDrawRedCount >= 0.4f)
	{
		m_redZone -= 0.8f;
		if (m_redZone <= 0.0f)
		{
