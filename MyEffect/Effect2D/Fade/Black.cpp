...
const std::vector<D3D11_INPUT_ELEMENT_DESC> Black::INPUT_LAYOUT =
{
	{ "POSITION",	0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "COLOR",		0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(Vector3), D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT, 0, sizeof(Vector3) + sizeof(Vector4), D3D11_INPUT_PER_VERTEX_DATA, 0 },
};
void Black::Create(DX::DeviceResources *& deviceResources, int count)
{
	...
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticlePSBlack.cso");
...
void Black::Initialize(float life, const DirectX::SimpleMath::Vector3 & pos)
{
	int range = 100;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void Black::InitializeNormal(float life, const DirectX::SimpleMath::Vector3 & pos)
{
	int range = 1000;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void Black::InitializeCorn(float life, const DirectX::SimpleMath::Vector3 & pos, const DirectX::SimpleMath::Vector3 & dir)
{
	int range = 10;
	Vector3 vel = Vector3::Zero;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
...
void Black::Update(const DX::StepTimer & timer)
{
	m_timer = timer;
	for(std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end();itr++)
	{
...
void Black::SetRenderState(const DirectX::SimpleMath::Vector3 & camera, const DirectX::SimpleMath::Matrix & view, const DirectX::SimpleMath::Matrix & proj)
{
	m_view = view;
	m_proj = proj;
...
void Black::Draw(const DirectX::SimpleMath::Matrix & world, const DirectX::SimpleMath::Matrix & view, const DirectX::SimpleMath::Matrix & proj)
{
	auto context = m_pDeviceResources->GetD3DDeviceContext();
	...
	//Time		x:経過時間(トータル秒)	y:1Fの経過時間(秒）	z:反復（サインカーブ） w:アルファ値
	cbuff.Time = Vector4(GameContext<DataManager>::Get()->GetTime(), float(m_timer.GetElapsedSeconds()), sinf(float(m_timer.GetTotalSeconds())), m_alpha);
...
void Black::SetAlpha(float alpha)
{
	m_alpha = alpha;
}