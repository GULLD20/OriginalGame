...
void Fade::Create(DX::DeviceResources* &deviceResources, int count)
{
	...
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticlePSFade.cso");
...
void Fade::Initialize(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	int range = 100;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void Fade::InitializeNormal(float life, const DirectX::SimpleMath::Vector3 &pos)
{
	int range = 1000;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
	{
...
void Fade::InitializeCorn(float life, const DirectX::SimpleMath::Vector3 &pos, const DirectX::SimpleMath::Vector3 &dir)
{
	int range = 10;
	Vector3 vel = Vector3::Zero;
	for (std::list<MyEffect*>::iterator itr = m_pEffectList.begin(); itr != m_pEffectList.end(); itr++)
...
void Fade::Draw(const DirectX::SimpleMath::Matrix &world, const DirectX::SimpleMath::Matrix &view, const DirectX::SimpleMath::Matrix &proj)
{
	...
	cbuff.Time = Vector4(float(m_timer.GetTotalSeconds()), float(m_timer.GetElapsedSeconds()), sinf(float(m_timer.GetTotalSeconds())), m_fadeTimming);
