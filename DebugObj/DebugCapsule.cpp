...
DebugCapsule::DebugCapsule(ID3D11Device * device, const DirectX::SimpleMath::Vector3 &a, const DirectX::SimpleMath::Vector3 &b, float radius)
{
	...
	// 頂点バッファの作成
	device->CreateBuffer(&desc, &data, m_pVertexBuffer[0].GetAddressOf());

	// 頂点バッファの作成（ヘッド）
	desc.ByteWidth = sizeof(XMFLOAT3) * VERTEX_CNT_HEAD;
	data.pSysMem = vertexes_head;
	device->CreateBuffer(&desc, &data, m_pVertexBuffer[1].GetAddressOf());
	...
	// インデックスバッファの作成
	device->CreateBuffer(&desc, &data, m_pIndexBuffer[0].GetAddressOf());

	// インデックスバッファの作成（ヘッド）
	desc.ByteWidth = sizeof(UINT) * INDEX_CNT_HEAD;
	data.pSysMem = indexes_head;
	device->CreateBuffer(&desc, &data, m_pIndexBuffer[1].GetAddressOf());
	...
	desc.CPUAccessFlags = 0;
	device->CreateBuffer(&desc, NULL, m_pConstantBuffer.GetAddressOf());

	// シェーダーの作成
	device->CreateVertexShader((void*)VertexShader, sizeof(VertexShader), NULL, m_pVertexShader.GetAddressOf());
	device->CreatePixelShader((void*)PixelShader, sizeof(PixelShader), NULL, m_pPixelShader.GetAddressOf());
	...
	device->CreateInputLayout(layout, _countof(layout), VertexShader, sizeof(VertexShader) / sizeof(BYTE), m_pInputLayout.GetAddressOf());
	...
	device->CreateDepthStencilState(&dsDesc, m_pDepthStencilState.GetAddressOf());
	...
	device->CreateRasterizerState(&rsDesc, m_pRasterizerState.GetAddressOf());
#endif
...
void DebugCapsule::Draw(ID3D11DeviceContext* context, CommonStates& states, const Matrix& world, const Matrix& view, const Matrix& projection)
{
	...
	// 頂点バッファ
	UINT vb_slot = 0;
	ID3D11Buffer* vb[2] = { m_pVertexBuffer[0].Get(), m_pVertexBuffer[1].Get() };
	UINT stride[1] = { sizeof(XMFLOAT3) };
	UINT offset[1] = { 0 };
	context->IASetVertexBuffers(vb_slot, 1, &vb[0], stride, offset);

	// インデックスバッファ
	context->IASetIndexBuffer(m_pIndexBuffer[0].Get(), DXGI_FORMAT_R32_UINT, 0);
	...
	context->UpdateSubresource(m_pConstantBuffer.Get(), 0, NULL, &cbuff, 0, 0);

	// 定数バッファ
	ID3D11Buffer* cb[1] = { m_pConstantBuffer.Get() };
	...
	context->IASetIndexBuffer(m_pIndexBuffer[1].Get(), DXGI_FORMAT_R32_UINT, 0);
...
void DebugCapsule::SetCenterSegment(const DirectX::SimpleMath::Vector3 &a, const DirectX::SimpleMath::Vector3 &b)
{
	m_a = a;
	m_b = b;
	Vector3 v = b - a;
